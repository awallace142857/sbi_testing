...
def simulator(theta):
    return torch.tensor(binary_color_mag_isochrones(*theta[0:4]))

def simulate_data_sets(theta):
    return torch.tensor(binary_color_mag_data_sets(*theta[0:4]))
# Set priors.
...
prior_no_dist = utils.BoxUniform(low=bounds[0:4].T[0], high=bounds[0:4].T[1])
from sbi.inference import prepare_for_sbi, simulate_for_sbi

sbi_simulator, sbi_prior = prepare_for_sbi(simulator, prior)

inference = method(prior_no_dist)
...
def simulate_for_sbi_strict(simulator, proposal, num_simulations, max_trials=np.inf):
	...
	while num_simulated < num_simulations:
		...
		_x = simulator(_theta[:,0:4])
		#_theta, _x = simulate_for_sbi(simulator, proposal=proposal, num_simulations=N)
		pars = 1000./_theta[:,4]
		x_arr = np.array(_x)
		app_mags = x_arr[:,0:8]
		for ii in range(8):
			app_mags[:,ii]+=5*np.log10(100./np.array(pars))
		errs = get_errors(app_mags)
		zeros = np.where(errs==0)[0]
		for ii in range(8):
			errs[:,ii+1] = np.sqrt(errs[:,ii+1]**2+((5*errs[:,0])/(np.array(pars)*np.log(10)))**2)
		x_arr[:,8:16] = errs[:,1:9]
		for ii in range(8):
			x_arr[:,ii]-=5*np.log10(100./np.array(pars))
		x_means = x_arr[:,0:8]
		for ii in range(8):
...
pickle.dump((theta, x), open('sbi_theta_x_with_dist.pkl', 'wb'))
#sys.exit()
(theta, x) = pickle.load(open('sbi_theta_x_with_dist.pkl', 'rb'))
#theta = torch.tensor(theta.astype('float32'))
#x = torch.tensor(x.astype('float32'))
#x = x[:,(0,1,2,3,9,10,11,12)]
density_estimator = inference.append_simulations(theta[:,0:4], x).train()
posterior = inference.build_posterior(density_estimator)
pickle.dump((posterior, (theta, x)), open('sbi_posterior_with_dist.pkl', 'wb'))
#sys.exit()"""
#(posterior, (theta, x)) = pickle.load(open(saveDir+'/sbi_posterior.pkl', 'rb'))"""
(posterior, (theta, x)) = pickle.load(open('sbi_posterior_with_dist.pkl', 'rb'))
...
def simulate_binaries(num_simulations):
	...
	while num_simulated < num_simulations:
		...
		textFile = open('current_'+str(N),'w')
		_theta = np.array([m1s,qs,ages,fehs,dists])
		_x = simulate_data_sets(_theta[0:4])
		pars = 1000./_theta.T[:,4]
		x_arr = np.array(_x)
		app_mags = x_arr[:,0:8]
		for ii in range(8):
			app_mags[:,ii]+=5*np.log10(100./np.array(pars))
...
pickle.dump((true_theta,true_obs),open('binary_data_with_dist.pkl','wb'))
sys.exit()"""
#(true_theta,true_obs) = pickle.load(open('binary_data.pkl','rb'))
#x = true_obs#[:,(0,1,2,3,9,10,11,12)]
(true_theta,true_obs) = pickle.load(open('binary_data_with_dist.pkl','rb'))
x = true_obs
#x[:,9:18] = 0
ii = 10
print(true_theta[ii],x[ii])
sample = posterior.sample(
		(num_samples,),
		x=x[ii],
		show_progress_bars=False
	)
pickle.dump((np.array(true_theta[ii]),np.array(x[ii]),np.array(sample)), open('sbi_ex_test_with_dist.pkl', 'wb'))
sys.exit()
