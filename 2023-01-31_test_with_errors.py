...
def binary_color_mag_isochrones(m1, q, age, fe_h, dist):
	...
	all_errs = 0.01*np.ones((1,9))
	all_errs[:,0] = 0#parallax error is zero
...
def noiseless_mags(m1, q, age, fe_h, dist):
	properties = tracks.generate_binary(
	m1,
	q * m1,
	np.log10(age) + 9,
...
pickle.dump((posterior, (theta, x)), open('sbi_posterior_with_errors.pkl', 'wb'))
#sys.exit()"""
#(posterior, (theta, x)) = pickle.load(open(saveDir+'/sbi_posterior.pkl', 'rb'))"""
(posterior, (theta, x)) = pickle.load(open('sbi_posterior_with_errors.pkl', 'rb'))
ii = 6
...
pickle.dump((np.array(theta[ii]),np.array(x[ii]),np.array(sample)), open('sbi_ex_test_with_errors.pkl', 'wb'))